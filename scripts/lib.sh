...
download_packages() {
    ...
    download_file() {
        ...
        while [ $retry -lt "${CONFIG[MAX_RETRIES]}" ]; do
            ...
            retry=$((retry + 1))
            log "WARNING" "Retry $retry/${CONFIG[MAX_RETRIES]} for $output"
            sleep "${CONFIG[RETRY_DELAY]}"
        done
        return 1
    }
    ...
    case "$source" in
        github)
            for entry in "${package_list[@]}"; do
                local IFS='|'
                read -r filename base_url mode <<< "$entry"
                ...
                # apk: strip noarch suffix (_all.apk -> .apk). Keep real arch (aarch64_generic).
                if [ "${PKG_EXT:-ipk}" == "apk" ] && [[ "$output_file" == *_all.apk ]]; then
                    mv "$output_file" "${output_file%_all.apk}.apk"
                fi
            done
;;
